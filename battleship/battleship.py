...
class Ship(object):
    ...
    def place(self, col, row, direction):
        ...
        """
        # taking in a colum, row and a legal direction (H, V - to upper)
        # append ship coords to coords list
        try:
            if direction.upper() == 'H' or direction.upper() == 'V':
                self.coords= []
                if direction.upper() == 'H':
                    for i in range(col, self._length + 1):
                        # orient the ship coords horizontally
                        # take into consideration the length of the ship
                        self.coords.append(tuple([i, row]))
                else:
                    for i in range(self._length):
                        # orient the ship coords vertically
                        self.coords.append(tuple([col, row + i]))
            else:
